...
try {
    $movimiento = TurnosBootstrap::registrarMovimiento()->ejecutar(
        $canalId,
        isset($payload['fecha']) ? $payload['fecha'] : '',
        isset($payload['tipo']) ? $payload['tipo'] : '',
        isset($payload['detalle']) ? $payload['detalle'] : '',
        isset($payload['monto']) ? $payload['monto'] : null,
        $username,
        $idempotencyKey
    );

    JsonResponse::enviar(array('data' => array('movimiento' => $movimiento)), 201);
