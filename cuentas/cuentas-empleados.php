...
<html lang="en">
    <head>
        ...
        <script>
            $(document).ready(function(e) {
                ...
                var valorGrafico = function(row) {
                    var saldo = parseFloat(row.saldo || 0);
                    return row.tipoSaldo === 'D' ? Math.abs(saldo) : saldo;
                };

                var etiquetaGrafico = function(row) {
                    if (row.tipoSaldo === 'D') {
                        var saldo = parseFloat(row.saldo || 0);
                        if (saldo > 0) {
                            return row.nombre + ' (Debemos)';
                        }
                        if (saldo < 0) {
                            return row.nombre + ' (Nos debe)';
                        }
                    }
                    return row.nombre;
                };
                ...
                var construirMapaColores = function(cuentas) {
                    var positivas = $.grep(cuentas || [], function(row) { return valorGrafico(row) > 0; });
                    positivas.sort(function(a, b) { return valorGrafico(b) - valorGrafico(a); });
                ...
                var renderGraficoSaldos = function(cuentas, mapaColores) {
                    ...
                    var etiquetas = $.map(principales, function(row) { return etiquetaGrafico(row); });
                    var valores = $.map(principales, function(row) { return valorGrafico(row); });
                    var colores = $.map(principales, function(row) { return mapaColores[row.id] || COLOR_OTRAS; });

                    if (resto.length) {
                        var totalResto = 0;
                        $.each(resto, function(_, row) { totalResto += valorGrafico(row); });
                    ...
                    $.each(positivas, function(_, row) { totalPositivas += valorGrafico(row); });
                    ...
                    var totalDeuda = 0;
                    $.each(cuentas || [], function(_, row) {
                        if (row.tipoSaldo === 'D') {
                            totalDeuda += parseFloat(row.saldo || 0);
                        }
                    });
                    var etiquetaSaldoDeuda = totalDeuda < 0 ? ' (Nos deben)' : ' (Debemos)';
                    $('#saldoDeudaGrafico').text(formatoMoneda(Math.abs(totalDeuda)) + etiquetaSaldoDeuda);
                    ...
                    $.each(positivas, function(index, row) {
                        var valor = valorGrafico(row);
                        var porcentaje = totalPositivas > 0 ? (valor / totalPositivas * 100) : 0;
                        var color = mapaColores[row.id] || COLOR_OTRAS;

                        $tbody.append(
                            '<tr>' +
                                '<td><span class="d-inline-block rounded-circle me-2" style="width:10px;height:10px;background:' + color + ';"></span>' + escapeHtml(etiquetaGrafico(row)) + '</td>' +
                                '<td class="text-end">' + escapeHtml(formatoMoneda(valor)) + '</td>' +
                ...
                var renderCards = function(cuentas, mapaColores) {
                    ...
                    $.each(cuentas, function(index, row) {
                        ...
                        var saldoNumerico = parseFloat(row.saldo || 0);
                        var esDeuda = row.tipoSaldo === 'D';
                        var saldoMostrado = '$ ' + row.saldoEtiqueta;
                        if (esDeuda) {
                            saldoMostrado = formatoMoneda(Math.abs(saldoNumerico));
                            if (saldoNumerico > 0) {
                                tipoSaldoEtiqueta += ' - Debemos';
                            } else if (saldoNumerico < 0) {
                                tipoSaldoEtiqueta += ' - Nos debe';
                            }
                        }
                            ...
                            '<div class="col">' +
                                '<div class="card mb-4 rounded-3 shadow-sm h-100" style="border: 1px solid ' + color + ';' + (esActiva ? '' : ' opacity: 0.6;') + '">' +
                                    ...
                                    '<div class="card-body text-center">' +
                                        '<h1 class="card-title pricing-card-title">' + escapeHtml(saldoMostrado) + '</h1>' +
                                        '<ul class="list-unstyled mt-3 mb-4">' +
                                            '<li><i class="fa-solid ' + icono + '" style="color: ' + color + ';"></i> ' + escapeHtml(tipoSaldoEtiqueta) + '</li>' +
                                            '<li><i class="fa-solid fa-id-badge" style="color: ' + color + ';"></i> ' + escapeHtml(row.empleadoNombre || 'Sin empleado') + '</li>' +
                                        '</ul>' +
                                        '<div class="d-flex justify-content-center gap-3">' +
                                            ...
                                            '<a href="view-cuenta.php?forwardOk=cuentas-empleados.php&id=' + escapeHtml(row.id) + '" class="text-primary" title="Ver"><i class="fa fa-fw fa-eye fa-lg"></i></a>' +
                                            ...
                                            '<a href="#" class="text-danger js-borrar-cuenta" data-id="' + escapeHtml(row.id) + '" data-saldo="' + escapeHtml(row.saldo) + '" title="Borrar"><i class="fa fa-fw fa-trash fa-lg"></i></a>' +
                                        '</div>' +
                                    '</div>' +
                                '</div>' +
                            '</div>'
                ...
                var cargarCuentas = function() {
                    ...
                    var url = '../bff/cuentas/control.php?ambitoUso=EMPLEADO' + (incluirInactivas ? '&incluirInactivas=1' : '');
                ...
                $('#cuentasCards').on('click', '.js-borrar-cuenta', function(e) {
                    e.preventDefault();
                    var id = $(this).data('id');
                    var saldo = Number($(this).data('saldo')) || 0;

                    if (Math.abs(saldo) >= 0.01) {
                        alert('Esta cuenta tiene saldo (' + formatoMoneda(saldo) + ') y no se puede inactivar. Compensala primero.');
                        return;
                    }

                    if (!confirm('Esta seguro de querer inactivar esta cuenta?')) {
                        return;
                    }
                    ...
                    $form.append($('<input>').attr({ type: 'hidden', name: 'forwardOk', value: 'cuentas-empleados.php' }));
    ...
    <body class="sb-nav-fixed">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Cuentas de Empleados</h1>
                        <ol class="breadcrumb mb-4">
                            <li class="breadcrumb-item">Personal</li>
                            <li class="breadcrumb-item">Cuentas</li>
                            <li class="breadcrumb-item active">Cuentas de Empleados</li>
                        </ol>
                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <div class="mb-2">
                                            <span class="text-muted">Saldo Deuda:</span>
                                            <span class="fw-bold" id="saldoDeudaGrafico">$ 0,00</span>
                                        </div>
        ...
        <?php include_once 'nuevoModalEmpleado.php'; ?>
